...
function TV_HangUpCtrl(uID,phoneNum) {
	TV_EnableHook(uID, FALSE, phoneNum);
}
//----------------------------------------
...
function TV_StartPlayFile(uID, szFile) {
	// vFilePath = "e:\\test.wav";
	vFilePath = "e:\phoneRecord\test.wav";
...
function TV_StartRecordFile(uID , phoneNum) {
	...
	// var stry = localStorage.getItem("caseNo") + "-"+localStorage.getItem("litigantName") +"-" + dates + ".mp3";
	var caseno = localStorage.getItem("caseNo")
	var casenoindex =caseno.indexOf("民初");
	var casenoindexend =caseno.indexOf("号");
	caseno = caseno.substring(casenoindex+2,casenoindexend);

	var stry =  caseno+'-'+ localStorage.getItem("phoneNums") +"-" + dates + ".mp3";//localStorage.getItem("caseNo") + "-"+
			...
			RecordFile(uID, QNV_RECORD_FILE_ELAPSE, uRecordID[uID], 0, 0, function(vElapse) {
				RecordFile(uID, QNV_RECORD_FILE_STOP, uRecordID[uID], 0, 0, function(nRet) {
					...
					window.setTimeout(function(){
						...
						TV_uploadFile('https://dq.hlcourt.gov.cn/api/main/telephone.jhtml?code='+stamps,phoneNum);
...
function TV_uploadFile(uploadUrl,phoneNum) {
	...
	Remote(QNV_REMOTE_UPLOAD_START, 0, uploadUrl, vFilePath, 0, 0, function(nRet) {
		...
		if(nRet <= 0)
			...
			AppendStatus("上传文件:" + vFilePath + "成功" + 'code:' + phoneNum + "【开始校验上传成功与否】");
