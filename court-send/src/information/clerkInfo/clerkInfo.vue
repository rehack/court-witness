<template>
    <div>
        ...
        <Card>
            <Row>
                <Col span="3">
                    <span style="color:#464c5b;font-size:16px;font-weight:700">书记员信息库</span>
                </Col>
                <Col span="21">
                    ...
                    <Form :model="formItem" inline :label-width="100" v-show="show1" @keydown.native.enter.prevent="getList(1)">
                        ...
                        <FormItem label="工作部门：">
                          <Select v-model="formItem.departmentName" clearable style="width:200px">
                              <Option v-for="item in departmentList" :value="item.name" :key="item.id">{{ item.name }}</Option>
                          </Select>
                        </FormItem>
                        <FormItem label="工作年限：">
  ...
  methods: {
    ...
    getList(page) {
      ...
      list(params)
        ...
        .catch(() => {
          that.$Message.error("网络错误, 查询书记员列表失败");
        });
    },
    addClerk() {
